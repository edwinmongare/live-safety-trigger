...
export function SmdView() {
  const [data, setData] = useState<Data[]>([]);

  useEffect(() => {
    const fetchData = () => {
      fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/SMDQuestions/?limit=10000`)
    ...
    <BentoGrid className="max-w-full mx-auto mt-5">
      {requiredLines.map((requiredLine) => {
        const lineData = data.find((item) =>
          item.Line.some((line) => line.smd_line_name === requiredLine)
        );

        if (lineData) {
          const line = lineData.Line.find(
            (line) => line.smd_line_name === requiredLine
          );
          ...
          if (line) {
            return (
              <Link
                key={line.id}
                href={`/Line/Safety/smd/${requiredLine}`}
