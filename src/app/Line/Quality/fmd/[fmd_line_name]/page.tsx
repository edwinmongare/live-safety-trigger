...
interface Product {
  ...
  updatedAt?: string;
  user?: { email: string }[];
  Line?: { FMD_Lines: string }[];
  // Define other properties as per your actual data structure
}

const Page = async ({ params }: PageProps) => {
  const { fmd_line_name } = params;
  const decodedLineName = decodeURIComponent(fmd_line_name);
  const payload = await getPayloadClient();

  const { docs: products } = await payload.find({
    collection: "FMD_Quality_Inspection",
    limit: 1,
    where: {
      "Line.FMD_Lines": {
