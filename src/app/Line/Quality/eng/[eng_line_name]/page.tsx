...
interface Product {
  ...
  updatedAt?: string;
  user?: { email: string }[];
  Line?: { eng_lines: string }[];
  // Define other properties as per your actual data structure
}

const Page = async ({ params }: PageProps) => {
  const { eng_line_name } = params;
  const decodedLineName = decodeURIComponent(eng_line_name);
  const payload = await getPayloadClient();

  const { docs: products } = await payload.find({
    collection: "Engineering_Quality_Inspection",
    limit: 1,
    where: {
      "Line.eng_lines": {
